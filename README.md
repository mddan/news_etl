...
8. In Databricks, mediastack_headlines landing table is read and the delta load workflow executes as follows:
   a. A delta article table is generated (only new article records is generated by only selecting article not yet in existing bronze table)
   b. A deduped list of sources from the delta article table is used to hit the sources API and extract the delta_sources table.
   c. Using existing bronze_articles, only new sources are added to bronze_sources, and where applicable include any new source-specific fields from delta_sources
   d. The updated bronze_articles and bronze_sources tables are enriched to silver_articles and silver_sources by enriching/transforming with country names, language names, date parsing, aggregating on article count by source, and adding useful boolean (e.g has-shock_value)
   e. Both silver tables is transformed to gold tables for both articles and sources - mainly renaming.
   f. And OBT table is formed from the 1 fact gold table (articles) and the 3 gold dim tables (sources, countries, languages).
9. At each transition between bronze -> silver -> gold -> obt, QC tests are performed via Great Expectation tests to ensure outputs are consistently as expected throughout the data flowl 
10. The final transformed and enriched gold dataset and obt table, are brought into a Power BI dashboard where the data is dimensionally modeled, and exposed as semantic layer to illustrate latest available headlines by news category, and some basic statistics/trends on the full available news data. 
